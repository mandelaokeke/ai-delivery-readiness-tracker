...
export default async function WorkspacePage() {
  ...
  if (configured) {
    ...
    if (memberOrganisation) {
      ...
      const { data } = await supabase.from("workstreams").select("id, organisation_id, name, owner_name, milestone, risks, status, severity, progress, due_date").eq("organisation_id", organisation.id).order("created_at", { ascending: false });
